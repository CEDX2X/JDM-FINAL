      ...
      <header className="relative pt-32 pb-20 overflow-hidden bg-surface-container-low">
        <div className="max-w-7xl mx-auto px-4 md:px-8 grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-12 items-center">
          <div className="z-10">
            ...
            <p className="text-lg text-on-surface-variant dark:text-white max-w-lg mb-10 leading-relaxed font-body">
              {t('sport.desc')}
            </p>
            <div className="flex space-x-4">
              <button 
                onClick={() => document.getElementById('kits')?.scrollIntoView({ behavior: 'smooth' })} 
                className="bg-primary-container text-on-primary px-4 md:px-8 py-4 rounded-xl font-bold text-lg scale-102 transition-transform shadow-lg"
              >
                {t('sport.explore')}
              </button>
              <button 
                onClick={() => document.getElementById('filters')?.scrollIntoView({ behavior: 'smooth' })} 
                className="border-2 border-outline-variant/30 text-primary dark:text-white px-4 md:px-8 py-4 rounded-xl font-bold text-lg hover:bg-surface transition-colors"
              >
                {t('sport.clubs')}
              </button>
            </div>
          ...
          <div className="relative h-64 md:h-80 lg:h-[500px] hidden lg:block">
            ...
            <div className="absolute -bottom-6 -left-6 bg-surface-container-lowest p-6 rounded-2xl shadow-xl border border-outline-variant/10 max-w-[240px]">
              ...
              <p className="text-xs text-on-surface-variant dark:text-white font-medium">{t('sport.auth_desc')}</p>
            </div>
          </div>
        </div>
      </header>
        ...
        <div className="flex-1 w-full mx-auto">
          <div id="kits" className="flex justify-between items-center mb-10">
            ...
            <div className="flex items-center gap-4 text-sm font-medium">
              <span className="text-on-surface-variant dark:text-white font-bold px-4 py-2 bg-surface-container-low rounded-xl">{t('sport.showing')}</span>
            </div>
          </div>
